...
// @version 2025.10.03
// @author converted
// @url http://45.79.102.135/allcp.org/
// @enabled true
// @tags 特殊,书源,小说,converted
// @description // Error: 发现失效
// @description ◎只看楼主：
// @description 在设置源变量（该源所有书应用）或者设置书籍变量（只有本书应用）里填上『只看楼主』则只会显示楼主内容，部分页数可能无正文
// @description ◎开启只看楼主并保留用户名和发表时间：
// @description 在设置源变量（该源所有书应用）或者设置书籍变量（只有本书应用）里再填上『保留』则会显示楼主的用户名和发表时间以及内容，部分页数可能无正文
// @description ❗️注意：楼主如果是『匿名青花鱼』则所有匿名青花鱼发的贴都会显示，正文如果缓存了需要刷新才有效，或者清除缓存
